...
local aliases = {}
for key, data in pairs(builtin_textobjects) do
        local pair = data[1] aliases[pair[2]] = key ~= pair[2] and data or nil
end
for alias, data in pairs(aliases) do
        builtin_textobjects[alias] = data
end
...
local function take_param(_, d)
	if d and type(d[3]) == "table" then
		if #d[3] == 2 then
			if table.concat(d[3]):find("\xef\xbf\xbd", 1, true) then
				local status, out = vis:pipe(nil, nil, "vis-menu" .. (d[4] and " -p '" .. d[4] .. ":'" or ""))
				if status == 0 then
...
local function outer(key)
        return builtin_textobjects[key] and builtin_textobjects[key].id or builtin_textobjects[1].id
end
...
local function operator_new(prefix, handler, nargs, help)
	...
	if type(prefix) == "table" then
		...
		if prefix[1] then vis:map(vis.modes.NORMAL, prefix[1], va_call(id, nargs, needs_range), h(help)) end
		if prefix[2] then vis:map(vis.modes.VISUAL, prefix[2], va_call(id, nargs), h(help)) end
	end
	return id
end
